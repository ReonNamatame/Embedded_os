//device driver for serial device
#include "defines.h"
#include "serial.h"
...
//definition of each SCI register
struct h8_3069f_sci {
    volatile uint8 smr;
    volatile uint8 brr;
    volatile uint8 scr;
...
#define H8_3069F_SCI_SCR_RE     (1<<4) //receive valid
#define H8_3069F_SCI_SCR_TE     (1<<5) //send valid
#define H8_3069F_SCI_SCR_RIE    (1<<6) //receive interrupt valid
#define H8_3069F_SCI_SCR_TiE    (1<<7) //send interrupt valid
...
#define H8_3069F_SCI_SSR_RDRF   (1<<6) //recieve complete
#define H8_3069F_SCI_SSR_TDRE   (1<<7) //send complete
...
int serial_init(int index)
{
    ...
    sci->scr = H8_3069F_SCI_SCR_RE | H8_3069F_SCI_SCR_TE; //send and receive enable
    sci->ssr = 0;
    
    return 0;
}

//check if sending can be done or not
...
int serial_send_byte(int index, unsigned char c)
{
    ...
    //wait until sending enables
    while (!serial_is_send_enable(index))
        ;
    sci->tdr = c;
    sci->ssr &= ~H8_3069F_SCI_SSR_TDRE; //sending begins
